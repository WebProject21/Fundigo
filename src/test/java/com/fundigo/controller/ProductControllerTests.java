...
import
 ...
 import com.fundigo.domain.ListVO; import com.fundigo.domain.ProductVO;
 ...
 import lombok.Setter; import lombok.extern.log4j.Log4j;
   ...
   log.info(mockMvc.perform(MockMvcRequestBuilders.get("/product/view")
   .param("pno", "10")) .andReturn() .getModelAndView().getModelMap()); }
   */
   @Test public void testCOMMList() throws Exception{
   log.info(mockMvc.perform(MockMvcRequestBuilders.get("/product/notice")
   .param("pno", "1")) .andReturn() .getModelAndView().getModelMap()); }
   
   /*
   @Test public void testRegister() throws Exception{
   
   ProductVO product = new ProductVO(); product.setTitle("에구구 | 개량 한복의 우아함");
   product.
   setDescription("전통 한복 명장님과 명인들의 봉제와 침선으로 만들어지는, 우리 옷 한복의 4계절 홈웨어, 일상복 한복집에서 느끼는 섬세함과 디테일을 담아, 우리 옷 한복의 경험을 선물합니다."
   ); product.setId("gildong"); product.setGoalDate("2021-04-26");
   product.setGoalPrice(30000000); product.setTag("D");
   ...
   for(int i = 0; i< 2; i++) { list.setTitle("세트 01");
   list.setContents("- 물품01\r\n- 물품02\r\n- 물품03"); list.setPrice(22000);
   list.setPno(product.getPno()); lists.add(i, list); }
   ...
   @Test public void testSponsor() throws Exception{
   log.info(mockMvc.perform(MockMvcRequestBuilders .get("/product/sponsor")
   .param("pno", "21")) .andReturn() .getModelAndView().getModelMap()); }
   
   
   @Test public void testBoardView() throws Exception{
   log.info(mockMvc.perform(MockMvcRequestBuilders .get("/product/boardView")
...
>>>>>>> 3e2c5ffc84d746c95f56a030d7d0f152266d0113
